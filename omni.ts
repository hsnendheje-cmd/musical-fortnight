...
function tunrNetPlayerIntoRoboMonke(player) {
    function amethsyTempSpawnItemAtPos(bareID: string, pos: any, rot: any): any {
        try {
...
Il2Cpp.perform(() => {
    ...
    function applyMenuBgTexture(obj) {
        try {
            ...
            try { mat.method("set_mainTexture").invoke(menuBgTexture); } catch (_) {}
            try {
                mat.method("SetColor", 2).overload("System.String", "UnityEngine.Color").invoke(Il2Cpp.string("_BaseColor"), white);
            } catch (_) {}
            try { mat.method("set_color").invoke(white); } catch (_) {}
        }
        catch (_) { }
    }
    ...
    TEMPLATE_MENU_BUTTONS.push(menuAddTo("credits", [
        ...
        { buttonText: "Byte - menu design", method: () => { try {
            ...
            catch (_) { } }, isTogglable: false, toolTip: "tooltip no workie" },
    ]));
...
menuButton("cum", () => {
    if (time <= pissCooldown) {
        return;
    }
    pissCooldown = time + 0.055;
    try {
        let bodyTf = null;
        try {
            bodyTf = getTransform(bodyCollider);
        } catch (_) {}
        if (!bodyTf || bodyTf.isNull?.()) {
            try {
                bodyTf = getTransform(instance);
            } catch (_) {}
        }
        if (!bodyTf || bodyTf.isNull?.())
            return;
        shootGoopGun(bodyTf, 56, 100, 1, 2.2, 5.2);
    } catch (_) {}
}, "tooltip no workie"),
    ...
    function spawnItemFromPrefab(prefab, pos, rot) {
        ...
        for (const src of [1, 0, hlNewSpawnSource()]) {
            try {
                const r = PrefabGen.method("SpawnItem", 5)
                    .overload("AnimalCompany.GrabbableItemPrefab", "UnityEngine.Vector3", "UnityEngine.Quaternion", "OnBeforeSpawned", "AnimalCompany.ItemSpawnSource")
                    .invoke(prefab, pos, rotation, NULL, src);
                if (r && !r.isNull?.())
                    return r;
            }
            catch (_) { }
        ...
        try {
            const r = PrefabGen.method("SpawnItem", 5).invoke(prefab, pos, rotation, NULL, 1);
            if (r && !r.isNull?.())
                return r;
        }
        catch (_) { }
    ...
    function spawnItemAtPos(bareID, pos, rot) {
        ...
        if (!prefab)
            return null;
        return spawnItemFromPrefab(prefab, pos, rot);
    }
...
function enableDeveloper() {
  try {
    ...
    try {
      ...
      if (user && !user.isNull?.()) {
        try { user.field('_isDeveloper').value = true; } catch {}
        try { user.field('isDeveloper').value = true; } catch {}
    ...
    try {
      ...
      if (mapMachine && !mapMachine.isNull?.()) {
        try {
          const devMode = mapMachine.method('get_useDevMode').invoke();
          if (devMode && !devMode.isNull?.()) {
            try { devMode.method('set_value').invoke(true); } catch {}
            try { devMode.field('_value').value = true; } catch {}
            try { devMode.field('value').value = true; } catch {}
          }
        } catch {}
        try { mapMachine.field('_useDevMode').value = true; } catch {}
    ...
    TEMPLATE_MENU_BUTTONS.push(menuAddTo("overpowered", [
        ...
        menuToggle("Money Printer", {
            onEnable: () => { moneyPrinterEnabled = true; sendNotification("don't work", false); },
            onDisable: () => { moneyPrinterEnabled = false; sendNotification("don't work", false); },
            onUpdate: () => {
                if (!moneyPrinterEnabled || time <= moneyPrinterDelay)
                    return;
                moneyPrinterDelay = time + 0.5;
                const lp = omniGetLocalNetPlayer();
                if (!lp)
                    return;
                try {
        ...
        menuToggle("Giant Me", {
            onEnable: () => {
                try {
                    const lp = omniGetLocalNetPlayer();
                    if (lp)
                        lp.method("set_playerScale").invoke(3.0);
                    sendNotification("don't work", false);
                }
                catch (_) { }
            },
            onDisable: () => {
                try {
                    const lp = omniGetLocalNetPlayer();
                    if (lp)
                        lp.method("set_playerScale").invoke(1.0);
                    sendNotification("don't work", false);
                }
                catch (_) { }
            },
        ...
        menuToggle("Tiny Me", {
            onEnable: () => {
                try {
                    const lp = omniGetLocalNetPlayer();
                    if (lp)
                        lp.method("set_playerScale").invoke(0.3);
                    sendNotification("don't work", false);
                }
                catch (_) { }
            },
        ...
        menuToggle("Big Head Me", {
            onEnable: () => {
                try {
                    const lp = omniGetLocalNetPlayer();
                    if (lp)
                        lp.method("set_playerHeadSize").invoke(4.0);
                    sendNotification("don't work", false);
                }
                catch (_) { }
            },
            onDisable: () => {
                try {
                    const lp = omniGetLocalNetPlayer();
                    if (lp)
                        lp.method("set_playerHeadSize").invoke(1.0);
                    sendNotification("don't work", false);
                }
                catch (_) { }
            },
        ...
        menuToggle("Cum Mod", {
            ...
            onUpdate: () => {
                ...
                try {
                    ...
                    const spawnPos = [
                        bodyPos.field("x").value + (fx * 0.3) + ((Math.random() * 0.06) - 0.03),
                        bodyPos.field("y").value - 0.4,
                        bodyPos.field("z").value + (fz * 0.3) + ((Math.random() * 0.06) - 0.03)
                    ];
                    const goop = spawnItemAtPos("item_goop", spawnPos, identityQuaternion);
                    if (goop && !goop.isNull()) {
                        spawnedGoopObjects.push({ object: goop, expireAt: time + 2.2 });
                        const vel = [
                            (fx * 10.0) + ((Math.random() * 1.5) - 0.75),
                            -0.5 + (Math.random() * 0.5),
                            (fz * 10.0) + ((Math.random() * 1.5) - 0.75)
                        ];
    ...
    TEMPLATE_MENU_BUTTONS.push(menuAddTo("users", [
        ...
        menuToggle("Chaos Teleport", {
            onEnable: () => {
                chaosTeleportEnabled = true;
                ...
                for (const p of players) {
                    try {
                        if (!p || p.handle.isNull())
                            continue;
                        const pos = getTransform(p).method("get_position").invoke();
                        originalPlayerPositions.set(p, pos);
                    }
                    catch (_) { }
                }
                sendNotification("don't work", false);
            ...
            onDisable: () => {
                chaosTeleportEnabled = false;
                let count = 0;
                originalPlayerPositions.forEach((pos, player) => {
                    try {
                ...
                originalPlayerPositions.clear();
                sendNotification("don't work" + count + " players.", false);
            },
            onUpdate: () => {
                if (!chaosTeleportEnabled)
                    return;
                if (time <= chaosTeleportDelay)
                    return;
                chaosTeleportDelay = time + 0.15;
                const players = getAllNetPlayersList(false);
                for (const p of players) {
                    try {
                        if (!p || p.handle.isNull())
                            continue;
                        const origPos = originalPlayerPositions.get(p);
                        if (!origPos || origPos.isNull())
                            continue;
                        const ox = origPos.field("x").value;
                        const oy = origPos.field("y").value;
                        const oz = origPos.field("z").value;
                        const rx = (Math.random() * 30) - 15;
                        const rz = (Math.random() * 30) - 15;
                        const newPos = [ox + rx, oy + Math.random() * 3, oz + rz];
                        p.method("RPC_Teleport").invoke(newPos);
                    }
                    catch (_) { }
                }
            },
    ...
    TEMPLATE_MENU_BUTTONS.push(menuAddTo("items", [
        ...
        menuToggle("Item ESP", {
            onEnable: () => { itemESPEnabled = true; },
            onDisable: () => {
                itemESPEnabled = false;
                for (const obj of itemESPObjects) {
                    try {
                        Destroy(obj);
                    }
                    catch (_) { }
            ...
            onUpdate: () => {
                ...
                for (const obj of itemESPObjects) {
                    try {
                        obj.method("SetActive").invoke(false);
                    }
                    catch (_) { }
                }
                const myPos = omniGetSafeSelfPosition();
                if (!myPos)
                    return;
                try {
                    const items = Object.method("FindObjectsByType", 1).inflate(GBIClass).invoke(0);
                    if (!items || items.isNull?.() || !items.length)
                        return;
                    let idx = 0;
                    for (let i = 0; i < items.length; i++) {
                        try {
                            const gi = items.get(i);
                            if (!gi || gi.isNull?.())
                                continue;
                            const sv = gi.method("get_sellValue").invoke();
                            if (typeof sv !== "number" || sv < 10)
                                continue;
                            const itemPos = getTransform(gi).method("get_position").invoke();
                            const d = Vector3.method("Distance").invoke(myPos, itemPos);
                            if (d > 30)
                                continue;
                            let espObj;
                            if (idx < itemESPObjects.length) {
                                espObj = itemESPObjects[idx];
                                espObj.method("SetActive").invoke(true);
                            }
                            else {
                                espObj = createObject(zeroVector, identityQuaternion, [0.06, 0.06, 0.06], 0, [0, 1, 0, 0.6]);
                                const lineObj = createObject(zeroVector, identityQuaternion, oneVector, 0, [0, 0, 0, 0]);
                                const lr = addComponent(lineObj, LineRenderer);
                                lr.method("set_startWidth").invoke(0.01);
                                lr.method("set_endWidth").invoke(0.003);
                                lr.method("set_positionCount").invoke(2);
                                lr.method("set_useWorldSpace").invoke(true);
                                const lc = [0, 1, 0, 0.4];
                                lr.method("set_startColor").invoke(lc);
                                lr.method("set_endColor").invoke(lc);
                                itemESPObjects.push(espObj, lineObj);
                            }
                            getTransform(espObj).method("set_position").invoke(itemPos);
                            if (idx + 1 < itemESPObjects.length) {
                                const lineESP = itemESPObjects[idx + 1];
                                const lr2 = getComponent(lineESP, LineRenderer);
                                if (lr2 && !lr2.isNull?.()) {
                                    lr2.method("SetPosition").invoke(0, myPos);
                                    lr2.method("SetPosition").invoke(1, itemPos);
                                }
                            }
                            idx += 2;
                        }
    ...
    const mmR = {
        addForce(o, v) { const [x, y, z] = mmVec3(v); try {
            o.method("RPC_AddForce", 1).invoke([x, y, z]);
        }
        catch (_) { } },
        playerDie(o, isDie) { try {
            o.method("RPC_DoPlayerDie").invoke(isDie);
        }
        catch (_) { } },
        teleport(o, v) { const [x, y, z] = mmVec3(v); try {
        ...
        playerStun(o, pos, range, dur, atten) { const [x, y, z] = mmVec3(pos); try {
            o.method("RPC_PlayerStun", 4).invoke([x, y, z], range, dur, atten | 0);
        }
        catch (_) { } },
        addExtForce(o, f, mode) { const [x, y, z] = mmVec3(f); try {
            o.method("RPC_AddExternalForce").invoke([x, y, z], mode | 0);
        }
        catch (_) { } },
        playerHit(o, dmg, pos, force, srcInfo, death) { const [x, y, z] = mmVec3(pos); try {
    ...
    function mmSetSqueaky(p, on) { if (p && !p.isNull?.()) {
        try {
    ...
    function hlSpawnItem(name, pos, rot) {
        ...
        try {
            const r = PrefabGen.method("SpawnItem", 5)
                .overload("System.String", "UnityEngine.Vector3", "UnityEngine.Quaternion", "OnBeforeSpawned", "AnimalCompany.ItemSpawnSource")
    ...
    function hlSpawnItem2(name, pos, rot) {
        ...
        try {
            ...
            const r = PrefabGen.method("SpawnItem", 5)
                .overload("AnimalCompany.GrabbableItemPrefab", "UnityEngine.Vector3", "UnityEngine.Quaternion", "OnBeforeSpawned", "AnimalCompany.ItemSpawnSource")
                .invoke(prefab, pos, rot, NULL, hlNewSpawnSource());
            if (r && !r.isNull?.())
                return r;
        }
        catch (_) { }
    ...
    try {
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (!goopItem || goopItem.isNull?.()) continue;

            spawnedGoopObjects.push({ object: goopItem, expireAt: time + lifetime });

            try { setColorHue(goopItem, hue); } catch (_) {}
            try { setColorSaturation(goopItem, saturation); } catch (_) {}
            ...
            try {
                ...
                if (gameObject && !gameObject.isNull?.()) {
                    try {
                        ...
                        if (grabbable && !grabbable.isNull?.()) {
                            try { setColorHue(grabbable, hue); } catch (_) {}
                            try { setColorSaturation(grabbable, saturation); } catch (_) {}
                        }
                    } catch (_) {}
                    try {
    ...
    TEMPLATE_MENU_BUTTONS.push(menuAddTo("combat", [
        ...
        menuButton("Freeze All", () => {
            let n = 0;
            for (const p of mmAllPlayers(false)) {
                try {
                    const pos = getTransform(p).method("get_position").invoke();
                    mmR.playerStun(p, pos, 999.0, 9999.0, 0);
                    n++;
                }
                catch (_) { }
            }
            sendNotification("don't work" + n + " players", false);
        }, "freezes every other player in place."),
    ...
    TEMPLATE_MENU_BUTTONS.push(menuAddTo("overpowered", [
        ...
        menuToggle("Kill Aura", {
            onEnable: () => { mmKillAuraOn = true; sendNotification("don't work", false); },
            onDisable: () => { mmKillAuraOn = false; sendNotification("don't work", false); },
            onUpdate: () => {
                if (!mmKillAuraOn || time <= mmKillAuraDelay)
                    return;
                mmKillAuraDelay = time + 0.3;
                let myPos;
                try {
                    myPos = readVec3Components(getTransform(bodyCollider).method("get_position").invoke());
                }
                catch (_) {
                    return;
                }
                ...
                for (const p of mmAllPlayers(false)) {
                    try {
                        const pp = readVec3Components(getTransform(p).method("get_position").invoke());
                        const d = Math.sqrt(((pp[0] - myPos[0]) ** 2) + ((pp[1] - myPos[1]) ** 2) + ((pp[2] - myPos[2]) ** 2));
                        if (d <= 8) {
                            const pos = getTransform(p).method("get_position").invoke();
                            mmR.playerHit(p, 50, pos, zeroVector, dn, false);
                        }
                    }
                    catch (_) { }
                }
        ...
        menuButton("Revive All", () => {
            let n = 0;
            for (const p of mmAllPlayers(false)) {
                try {
                    mmR.playerDie(p, false);
                    n++;
                }
                catch (_) { }
            }
            sendNotification("don't work" + n + " players", false);
        }, "revive every other player once."),
        menuToggle("Revive All Auto", {
            onEnable: () => { mmReviveAutoOn = true; sendNotification("don't work", false); },
            onDisable: () => { mmReviveAutoOn = false; sendNotification("don't work", false); },
            onUpdate: () => {
                if (!mmReviveAutoOn || time <= mmReviveAutoDelay)
                    return;
                mmReviveAutoDelay = time + 1.0;
                for (const p of mmAllPlayers(false)) {
                    try {
                        mmR.playerDie(p, false);
                    }
    ...
    TEMPLATE_MENU_BUTTONS.push(menuAddTo("world", [
        ...
        menuToggle("Chaos Forces", {
            onEnable: () => { mmChaosOn = true; sendNotification("don't work", false); },
            onDisable: () => { mmChaosOn = false; sendNotification("don't work", false); },
            onUpdate: () => {
                if (!mmChaosOn || time <= mmChaosDelay)
                    return;
                mmChaosDelay = time + 0.4;
                for (const p of mmAllPlayers(false)) {
                    try {
                        mmR.addForce(p, [(Math.random() * 1600) - 800, (Math.random() * 900) + 200, (Math.random() * 1600) - 800]);
                    }
                    catch (_) { }
                }
            },
    ...
    const WORLD_MOBS_CATEGORY_BUTTONS = [
        menuButton("Kill All Mobs", () => {
            let n = 0;
            for (const h of mmFindAll("AnimalCompany.HordeMobController")) {
                try {
                    h.method("KillAllMobs", 1).invoke(true);
                    n++;
                }
                catch (_) {
                    try {
                        h.method("RPC_Reset").invoke();
                        n++;
                    }
                    catch (__) {
                        try {
                            h.method("Reset").invoke();
                            n++;
                        }
                        catch (___) { }
                    }
            ...
            sendNotification("don't work" + n + " hordes", false, 2);
        }, "kill every mob in the map."),
        menuButton("Close All Doors", () => {
            let n = 0;
            for (const d of mmFindAll("AnimalCompany.AreaDoorsNetObject")) {
                try {
                    d.method("CloseAllDoors").invoke();
                    n++;
                }
                catch (_) { }
            }
            sendNotification("don't work" + n, false, 2);
        }, "slam every area door shut."),
    ...
    TEMPLATE_MENU_BUTTONS.push(menuAddTo("items", [
        menuButton("Spawn x10 at Hand", () => {
            try {
                const base = readVec3Components(rightHandTransform.method("get_position").invoke());
                const rot = rightHandTransform.method("get_rotation").invoke();
                for (let i = 0; i < 10; i++) {
                    spawnItemAsync(selectedItemID, [base[0] + (Math.random() * 0.4 - 0.2), base[1] + 0.2 + i * 0.05, base[2] + (Math.random() * 0.4 - 0.2)], rot);
                }
                sendNotification("don't work" + selectedItemID, false);
            }
            catch (e) {
        ...
        }, "spawn 10 of the selected item at your hand."),
        menuButton("Item Ring (Self)", () => {
            try {
                const base = readVec3Components(getTransform(bodyCollider).method("get_position").invoke());
                const count = 12, r = 2.0;
                for (let i = 0; i < count; i++) {
                    const a = (Math.PI * 2 / count) * i;
                    spawnItemAsync(selectedItemID, [base[0] + Math.cos(a) * r, base[1] + 1.0, base[2] + Math.sin(a) * r], identityQuaternion);
                }
                sendNotification("don't work" + count + "x " + selectedItemID, false);
            }
            catch (e) {
                
            }
        }, "spawn a ring of the selected item around you."),
        menuButton("Grenade at Hand", () => {
            try {
                const pos = rightHandTransform.method("get_position").invoke();
        ...
        }, "drop a live grenade at your hand."),
        ...
        menuToggle("Item Rain (Self)", {
            onUpdate: () => {
                if (time <= mmItemRainDelay)
                    return;
                mmItemRainDelay = time + 0.25;
                try {
                    const base = readVec3Components(getTransform(headCollider).method("get_position").invoke());
                    spawnItemAsync(itemIDs[Math.floor(Math.random() * itemIDs.length)], [base[0] + (Math.random() * 6 - 3), base[1] + 6, base[2] + (Math.random() * 6 - 3)], identityQuaternion);
                }
                catch (_) { }
            },
            toolTip: "tooltip no workie"
            ...
            try {
                ...
                if (vA) {
                    vA.implementation = function (...vB: any[]) {
                        if (antiKickEnabled) { sendNotification("don't work", false); return; }
                    ...
                    try {
                        ...
                        if (vE) {
                            vE.implementation = function (...vF: any[]) {
                                if (antiBanEnabled) { sendNotification("don't work", false); return; }
    ...
    function blockAllRpcs() {
        if (blockKickEnabled) return;
        blockKickEnabled = true;
        try {
            const vA = (vG: any) => { try { if (vG === undefined || vG === null) return ""; return String(vG); } catch (_) { return ""; } };
            ...
            const vE = (vG: any) => {
                try {
                    const vH = vA(vG?.name);
                    if (!vC(vH)) return;
                    if (!vG || (vG as any).__antiRpcBlockHooked) return;
                    const vI = vG;
                    (vG as any).__antiRpcBlockHooked = true;
                    vI.implementation = function (...vK: any[]) {
                        if (blockRPCsEnabled) return;
            ...
            const vF = (vG: any) => {
                try { for (const vH of vG?.classes ?? []) { try { for (const vI of vH?.methods ?? []) { vE(vI); } } catch (_) {} } } catch (_) {}
            };
            try { for (const vG of Il2Cpp.domain.assemblies ?? []) { try { vF(vG.image); } catch (_) {} } } catch (_) {
                for (const vJ of ["AnimalCompany", "Assembly-CSharp", "Fusion.Runtime"]) { try { vF(Il2Cpp.domain.assembly(vJ).image); } catch (_) {} }
            }
            try {
                ...
                for (const vM of ["RPC_KickPlayer", "RPC_RefreshPrivateRoomInfo", "RPC_NotifyYeetStarted", "RPC_KickPlayer@Invoker", "RPC_RefreshPrivateRoomInfo@Invoker", "RPC_NotifyYeetStarted@Invoker"]) {
                    for (const vN of [0, 1, 2, 3, 4]) { try { vE(vL.method(vM, vN)); } catch (_) {} }
                    try { vE(vL.method(vM)); } catch (_) {}
                }
            } catch (_) {}
        } catch (_) { blockKickEnabled = false; }
    }

    const SAFETY_CATEGORY_BUTTONS: ButtonInfoConfig[] = [
        menuToggle("Anti Ban", {
    ...
    let cachedGrabbables: any = null;
    let randomAllCooldown = 0;
    let randomAllRotIndex = 0;
    const gripToggleState: Record<string, boolean> = {};

    function checkGripToggle(key: string): boolean {
        if (!leftGrab) { gripToggleState[key] = false; return false; }
        if (gripToggleState[key]) return false;
        gripToggleState[key] = true;
        return true;
    }

    function getLocalHeldItem(preferRight = true): any {
        try {
            const lp = NetPlayer.method("get_localPlayer").invoke();
    ...
    function setColorHue(obj: any, val: number) {
        if (!obj || obj.isNull?.()) return;
        const v = Math.max(-128, Math.min(127, val));
        try { obj.method("SetColorHueData").invoke(v); } catch (_) { try { obj.method("set_colorHue").invoke(v); } catch (_) {} }
        try { obj.method("HandleColorHueChanged").invoke(); } catch (_) {}
    }

    function setColorSaturation(obj: any, val: number) {
        if (!obj || obj.isNull?.()) return;
        const v = Math.max(-128, Math.min(127, val));
        try { obj.method("SetColorSaturationData").invoke(v); } catch (_) { try { obj.method("set_colorSaturation").invoke(v); } catch (_) {} }
        try { obj.method("HandleColorSaturationChanged").invoke(); } catch (_) {}
    }

    function getAllGrabbables(): any {
    ...
    function resolveItemPrefabName(name: string, pos: any): any {
    ...
    const pendingBagFills: { bag: any, origin: any, frames: number, hue: number, sat: number, scale: number, itemID?: string, itemIDs?: string[], itemCount?: number, label?: string, itemsSpawned: boolean }[] = [];

    function getRandomItemName(): string { return randomFillItems[Math.floor(Math.random() * randomFillItems.length)]; }

    function processBagFills() {
        for (let i = pendingBagFills.length - 1; i >= 0; i--) {
            const entry = pendingBagFills[i];
            entry.frames--;
            if (entry.frames <= 0 || entry.itemsSpawned) { pendingBagFills.splice(i, 1); continue; }
            if (entry.frames % 3 !== 0) continue;
            try {
                    ...
                    try {
                        ...
                        if (go && !go.isNull?.()) {
                            ...
                            if (gbo && !gbo.isNull?.()) {
                                ...
                                try { gbo.method("set_scaleModifier").invoke(entry.scale); } catch (_) {}
                            }
                        }
                    } catch (_) {}
                    try {
                        const bag = entry.bag;
                        if (bag && !bag.isNull?.()) {
                            try { bag.method("AddToBag").invoke(spawned); } catch (_) {
                                try { bag.method("AddToBagInternal").invoke(spawned); } catch (_) {}
                            }
                        }
                    } catch (_) {}
                }
                if (entry.frames <= count) entry.itemsSpawned = true;
            } catch (_) {}
        }
    }
    ...
    TEMPLATE_MENU_BUTTONS.push(menuAddTo("items", [
        menuToggle("No Gravity Held Item", {
            onUpdate: () => {
                const held = getLocalHeldItem();
                if (!held) return;
                try {
                    const rb = held.field("_rigidbody")?.value;
                    if (rb && !rb.isNull?.()) rb.method("set_useGravity").invoke(false);
                } catch (_) {}
            },
            toolTip: "tooltip no workie"
        }),
        menuToggle("No Gravity All Items", {
            onUpdate: () => {
                itemFrameCounter++;
                const all = getAllGrabbables();
                if (!all) return;
                if (itemFrameCounter % 5 !== 0) return;
                try {
                    for (let i = 0; i < all.length; i++) {
                        const obj = all.get(i);
                        if (!obj || obj.isNull?.()) continue;
                        try {
                            const rb = obj.field("_rigidbody")?.value;
                            if (rb && !rb.isNull?.()) rb.method("set_useGravity").invoke(false);
                        } catch (_) {}
                    }
                } catch (_) {}
        ...
        menuToggle("Scale All Items", {
            onUpdate: () => {
                itemScaleVal += 15;
                if (itemScaleVal > 127) itemScaleVal = -127;
                if (!rightGrab) return;
                const all = getAllGrabbables();
                if (!all) return;
                try {
                    for (let i = 0; i < all.length; i++) {
                        const obj = all.get(i);
                        if (obj && !obj.isNull?.()) try { obj.method("set_scaleModifier").invoke(itemScaleVal); } catch (_) {}
                    }
                } catch (_) {}
            },
            toolTip: "tooltip no workie"
        }),
        menuButton("Add to Bag", () => {
            try {
                const BackpackClass = acImage.class("AnimalCompany.BackpackItem");
                const bags = Object.method("FindObjectsByType", 1).inflate(BackpackClass).invoke(0);
                if (!bags || bags.isNull?.()) { sendNotification("don't work", false); return; }
                let count = 0;
                for (let i = 0; i < bags.length; i++) {
                    ...
                    for (let j = 0; j < 23; j++) {
                        try {
                            const pos = bag.method("get_transform").invoke().method("get_position").invoke();
                            const item = spawnItemAsync(getRandomItemName(), pos, identityQuaternion);
                            if (item && !item.isNull?.()) {
                                try { bag.method("AddToBag").invoke(item); } catch (_) { try { bag.method("AddToBagInternal").invoke(item); } catch (_) {} }
                                count++;
                            }
                        } catch (_) {}
                    }
                ...
                sendNotification("don't work", false);
            } catch (_) { sendNotification("don't work", false); }
        }, "Fills all bags in the scene with random items."),
        menuToggle("Custom Hue Held", {
            onUpdate: () => {
                const held = getLocalHeldItem();
                if (!held) return;
                if (rightGrab) { itemHueVal++; if (itemHueVal > 127) itemHueVal = -127; }
                if (leftGrab) { itemHueVal--; if (itemHueVal < -127) itemHueVal = 127; }
        ...
        menuToggle("Custom Saturation Held", {
            onUpdate: () => {
                const held = getLocalHeldItem();
                if (!held) return;
                if (rightGrab) { itemSatVal++; if (itemSatVal > 127) itemSatVal = -127; }
                if (leftGrab) { itemSatVal--; if (itemSatVal < -127) itemSatVal = 127; }
        ...
        menuToggle("Custom Scale Held", {
            onUpdate: () => {
                const held = getLocalHeldItem();
                if (!held) return;
                if (rightGrab) { itemScaleVal++; if (itemScaleVal > 127) itemScaleVal = -127; }
                if (leftGrab) { itemScaleVal--; if (itemScaleVal < -127) itemScaleVal = 127; }
                try { held.method("set_scaleModifier").invoke(itemScaleVal); } catch (_) {}
            },
            toolTip: "tooltip no workie"
        }),
        menuToggle("Rainbow Held Item", {
            onUpdate: () => {
                const held = getLocalHeldItem();
                if (!held) return;
                if (rightGrab) { itemHueVal++; itemSatVal++; }
                if (leftGrab) { itemHueVal--; itemSatVal--; }
                if (itemHueVal > 127) itemHueVal = -127;
                if (itemHueVal < -127) itemHueVal = 127;
                if (itemSatVal > 127) itemSatVal = -127;
                setColorHue(held, itemHueVal);
                setColorSaturation(held, itemSatVal);
            },
            toolTip: "tooltip no workie"
        ...
        menuToggle("Rainbow All Items", {
            onUpdate: () => {
                itemFrameCounter++;
                if (rightGrab) { itemHueVal += 5; itemSatVal += 5; }
                if (leftGrab) { itemHueVal -= 5; itemSatVal -= 5; }
                if (itemHueVal > 127) itemHueVal = -127;
                if (itemHueVal < -127) itemHueVal = 127;
                if (itemSatVal > 127) itemSatVal = -127;
                if (itemFrameCounter % 5 !== 0) return;
        ...
        menuToggle("Random Held Item", {
            onUpdate: () => {
                const held = getLocalHeldItem();
                if (!held) return;
                if (rightGrab) { itemHueVal++; itemSatVal++; itemScaleVal++; }
                if (leftGrab) { itemHueVal--; itemSatVal--; itemScaleVal--; }
                if (itemHueVal > 127) itemHueVal = -127;
                if (itemSatVal > 127) itemSatVal = -127;
                if (itemScaleVal > 127) itemScaleVal = -127;
                setColorHue(held, itemHueVal);
        ...
        menuToggle("Fast Random Held Item", {
            onUpdate: () => {
                const held = getLocalHeldItem();
                if (!held) return;
                if (rightGrab) { itemHueVal += 10; itemSatVal += 10; itemScaleVal += 10; }
                if (leftGrab) { itemHueVal -= 10; itemSatVal -= 10; itemScaleVal -= 10; }
                if (itemHueVal > 127) itemHueVal = -127;
                if (itemHueVal < -127) itemHueVal = 127;
                if (itemSatVal > 127) itemSatVal = -127;
                if (itemSatVal < -127) itemSatVal = 127;
        ...
        menuToggle("Random All Items", {
            onUpdate: () => {
                if (time <= randomAllCooldown) return;
                ...
                for (let i = 0; i < count; i++) {
                    ...
                    try { obj.method("set_scaleModifier").invoke(Math.max(-18, Math.min(62, sc))); } catch (_) {}
                }
        ...
        menuToggle("Galaxy Giveaway Gun", {
            onUpdate: () => {
                if (!rightGrab) return;
                const gunData = omniRenderGun();
                if (!leftGrab || !gunData.ray || gunData.ray.isNull?.()) return;
                if (!checkGripToggle("galaxy")) return;
                const items = [
                    "item_plunger", "item_cutie_dead", "item_randombox_modules", "item_landmine",
                    "item_zipline_gun", "item_pelican_case", "item_wood_log", "item_ac_cola",
                    "item_apple", "item_backpack_fish", "item_banana_chips", "item_baseball_bat",
                    "item_beans", "item_boombox", "item_box_fan", "item_brainslug_green",
                    "item_broom_halloween", "item_bubble_gun", "item_burrito", "item_ceo_plaque",
                    "item_company_ration_heal", "item_cracker", "item_flashlight_mega",
                    "item_floppy_5", "item_floppy_3", "item_gameboy", "item_whoopie",
                    "item_backpack_large_clover", "item_backpack_monkey", "item_banana",
                    "item_boombox_fishing", "item_cola", "item_drill", "item_egg",
                    "item_fish_big_shark", "item_treestick", "item_rpg_cny", "item_goldbar",
                    "item_pickaxe_cny", "item_pumpkin_pie", "item_ukulele_gold", "item_umbrella",
                ];
                try {
                    const hitPoint = gunData.ray.method("get_point").invoke();
                    for (const name of items) {
                        try {
                            const spawned = spawnItemAsync(name, hitPoint, identityQuaternion);
                            if (spawned && !spawned.isNull?.()) {
                                try {
                                    const go = spawned.method("get_gameObject").invoke();
                                    const gbo = go.method("GetComponentInChildren", 0).inflate(GBOClass).invoke();
        ...
        menuToggle("Prune Giveaway", {
            onUpdate: () => {
                if (!rightGrab) return;
                const gunData = omniRenderGun();
                if (!leftGrab || !gunData.ray || gunData.ray.isNull?.()) return;
                if (!checkGripToggle("prune")) return;
                try {
                    const hitPoint = gunData.ray.method("get_point").invoke();
                    for (let i = 0; i < 29; i++) {
                        try {
                            const spawned = spawnItemAsync("item_prune_juice", hitPoint, identityQuaternion);
                            if (spawned && !spawned.isNull?.()) {
                                try {
                                    const go = spawned.method("get_gameObject").invoke();
                                    const gbo = go.method("GetComponentInChildren", 0).inflate(GBOClass).invoke();
                                    if (gbo && !gbo.isNull?.()) {
                                        gbo.method("set_scaleModifier").invoke(Math.floor(Math.random() * 256) - 128);
                                        setColorHue(gbo, Math.floor(Math.random() * 255) - 127);
                                        setColorSaturation(gbo, Math.floor(Math.random() * 148) - 20);
                                    }
                                } catch (_) {}
                            }
                        } catch (_) {}
        ...
        menuToggle("Giveaway Bag Gun", {
            onUpdate: () => {
                processBagFills();
                if (!rightGrab) return;
                const gunData = omniRenderGun();
                if (!gunData.ray || gunData.ray.isNull?.()) return;
                if (!checkGripToggle("giveaway")) return;
                try {
                    const hitPoint = gunData.ray.method("get_point").invoke();
                    const bagName = bagPrefabs[Math.floor(Math.random() * bagPrefabs.length)];
                    const bag = spawnItemAsync(bagName, hitPoint, identityQuaternion);
                    if (!bag || bag.isNull?.()) { sendNotification("don't work", false); return; }
                    pendingBagFills.push({
                        bag, origin: hitPoint, frames: 120,
                        hue: Math.floor(Math.random() * 255) - 127,
                        sat: Math.floor(Math.random() * 255) - 127,
                        scale: Math.floor(Math.random() * 80) + 30,
                        itemsSpawned: false,
                    });
                } catch (_) { sendNotification("don't work", false); }
            },
        ...
        menuToggle("Quiver Gun", {
            onUpdate: () => {
                processBagFills();
                if (!rightGrab) return;
                const gunData = omniRenderGun();
                if (!leftGrab || !gunData.ray || gunData.ray.isNull?.()) return;
                if (!checkGripToggle("quiver")) return;
                try {
                    const hitPoint = gunData.ray.method("get_point").invoke();
                    const bag = spawnItemAsync("item_quiver", hitPoint, identityQuaternion);
                    if (!bag || bag.isNull?.()) { sendNotification("don't work", false); return; }
                    pendingBagFills.push({
                        bag, origin: hitPoint, frames: 600,
                        hue: Math.floor(Math.random() * 256), sat: Math.floor(Math.random() * 256) - 128,
                        scale: Math.floor(Math.random() * 256) - 128, itemCount: 10,
                        itemsSpawned: false,
                    });
                } catch (_) { sendNotification("don't work", false); }
            },
        ...
        menuToggle("Suitcase Giveaway Bags", {
            onUpdate: () => {
                processBagFills();
                if (!rightGrab) return;
                const gunData = omniRenderGun();
                if (!gunData.ray || gunData.ray.isNull?.()) return;
                if (!checkGripToggle("suitcase")) return;
                try {
                    const hitPoint = gunData.ray.method("get_point").invoke();
                    const bagName = bagPrefabs[Math.floor(Math.random() * bagPrefabs.length)];
                    const bag = spawnItemAsync(bagName, hitPoint, identityQuaternion);
                    if (!bag || bag.isNull?.()) { sendNotification("don't work", false); return; }
                    pendingBagFills.push({
                        bag, origin: hitPoint, frames: 90,
                        hue: Math.floor(Math.random() * 255) - 127,
                        sat: Math.floor(Math.random() * 255) - 127,
                        scale: Math.floor(Math.random() * 80) + 30,
                        itemID: "item_pelican_case", itemsSpawned: false,
                    });
                } catch (_) { sendNotification("don't work", false); }
            },
            toolTip: "tooltip no workie"
        }),
        menuToggle("Arena Gun Bag", {
            onUpdate: () => {
                processBagFills();
                if (!rightGrab) return;
                const gunData = omniRenderGun();
                if (!gunData.ray || gunData.ray.isNull?.()) return;
                if (!checkGripToggle("arena")) return;
                try {
                    const hitPoint = gunData.ray.method("get_point").invoke();
                    const bagName = bagPrefabs[Math.floor(Math.random() * bagPrefabs.length)];
                    const bag = spawnItemAsync(bagName, hitPoint, identityQuaternion);
                    ...
                    pendingBagFills.push({
                        ...
                        sat: Math.floor(Math.random() * 255) - 127,
                        scale: Math.floor(Math.random() * 80) + 30,
                        itemIDs: ["item_arena_pistol", "item_arena_shotgun"], itemsSpawned: false,
                    });
                } catch (_) { sendNotification("don't work", false); }
            },
            toolTip: "tooltip no workie"
        }),
        menuToggle("Landmine Giveaway", {
            onUpdate: () => {
                processBagFills();
                if (!rightGrab) return;
                const gunData = omniRenderGun();
                if (!gunData.ray || gunData.ray.isNull?.()) return;
                if (!checkGripToggle("landmine")) return;
                try {
                    const hitPoint = gunData.ray.method("get_point").invoke();
                    const bagName = bagPrefabs[Math.floor(Math.random() * bagPrefabs.length)];
                    const bag = spawnItemAsync(bagName, hitPoint, identityQuaternion);
                    ...
                    pendingBagFills.push({
                        ...
                        sat: Math.floor(Math.random() * 255) - 127,
                        scale: Math.floor(Math.random() * 80) + 30,
                        itemID: "item_landmine", itemsSpawned: false,
                    });
                } catch (_) { sendNotification("don't work", false); }
            },
            toolTip: "tooltip no workie"
        }),
        menuToggle("New Golden Items Bag", {
            onUpdate: () => {
                processBagFills();
                if (!rightGrab) return;
                const gunData = omniRenderGun();
                if (!gunData.ray || gunData.ray.isNull?.()) return;
                if (!checkGripToggle("golden")) return;
                try {
                    const hitPoint = gunData.ray.method("get_point").invoke();
                    const bagName = bagPrefabs[Math.floor(Math.random() * bagPrefabs.length)];
                    const bag = spawnItemAsync(bagName, hitPoint, identityQuaternion);
                    ...
                    pendingBagFills.push({
                        ...
                        sat: Math.floor(Math.random() * 255) - 127,
                        scale: Math.floor(Math.random() * 80) + 30,
                        itemIDs: ["item_backpack_gold", "item_crowbar_gold", "item_hookshot_gold", "item_sticky_dynamite_gold"],
                        itemsSpawned: false,
                    });
                } catch (_) { sendNotification("don't work", false); }
            },
        ...
        menuToggle("Bags in Bags", {
            onUpdate: () => {
                processBagFills();
                if (!rightGrab) return;
                const gunData = omniRenderGun();
                if (!leftGrab || !gunData.ray || gunData.ray.isNull?.()) return;
                if (!checkGripToggle("bagsinbags")) return;
                try {
                    const hitPoint = gunData.ray.method("get_point").invoke();
                    const bag = spawnItemAsync("item_backpack_large_base", hitPoint, identityQuaternion);
                    if (!bag || bag.isNull?.()) { sendNotification("don't work", false); return; }
                    for (let depth = 0; depth < 20; depth++) {
                        try {
                            const innerBag = spawnItemAsync("item_backpack_large_base", hitPoint, identityQuaternion);
                            if (innerBag && !innerBag.isNull?.()) {
                                try { bag.method("AddToBag").invoke(innerBag); } catch (_) { try { bag.method("AddToBagInternal").invoke(innerBag); } catch (_) {} }
                            }
                        } catch (_) {}
                    }
                    sendNotification("don't work", false);
        ...
        new ButtonInfo({
            buttonText: "Spawn Cola Machine",
            method: () => {
                try {
                    const position = rightHandTransform.method("get_position").invoke();
                    const rotation = Quaternion.method("Euler").invoke(0.0, 0.0, 0.0);
                    spawnNetworkPrefab("Vending_Machine_AC_Cola", position, rotation);
                } catch (_) { sendNotification("don't work", false); }
            },
            isTogglable: false,
            toolTip: "tooltip no workie"
        }),
        menuToggle("Cola Machine Gun", {
            onUpdate: () => {
                if (!rightGrab) return;
                const gunData = omniRenderGun();
                if (!leftGrab || !gunData.ray || gunData.ray.isNull?.()) return;
                if (!checkGripToggle("colamachine")) return;
                try {
                    const hitPoint = gunData.ray.method("get_point").invoke();
                    const rotation = Quaternion.method("Euler").invoke(0.0, 0.0, 0.0);
                    spawnNetworkPrefab("Vending_Machine_AC_Cola", hitPoint, rotation);
                } catch (_) { sendNotification("don't work", false); }
            },
            toolTip: "tooltip no workie"
        }),
        new ButtonInfo({
            buttonText: "Anti Gravity All",
            method: () => {
                try {
                    if (noGravAllCooldown <= Date.now() / 1000) {
                        noGravAllCooldown = Date.now() / 1000 + 5;
                        const enumerator = NetPlayer.field("playerIDToNetPlayer").value.method("get_Values").invoke().method("GetEnumerator").invoke();
                        while (enumerator.method("MoveNext").invoke()) {
                            ...
                            const headTransform = player?.field("head").value || null;
                            const _obj = spawnItemAtPos("item_anti_gravity_grenade", headTransform.method("get_position").invoke(), headTransform.method("get_rotation").invoke());
                            const _root = _obj.method("get_gameObject").invoke();
                            const _go = _root.method("get_transform").invoke().method("GetChild").invoke(0).method("get_gameObject").invoke();
                            const _he = _go.method("GetComponent", 1).inflate(Il2Cpp.domain.assembly("AnimalCompany").image.class("AnimalCompany.HandExplosive")).invoke();
                            _he.method("RPC_Use").invoke(false, false);
                            _he.method("set_isPaused").invoke(false);
                            _he.method("set_isPinPulled").invoke(true);
                            _he.method("ForceExplode").invoke();
                        }
                    }
                } catch (_) { sendNotification("don't work", false); }
            },
